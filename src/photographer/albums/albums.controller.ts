...
export class AlbumsController {
  ...
  async createAlbum(@Body() dto: AlbumsDto) {
    this.logger.log(
      `Received request to create a new album with data: ${JSON.stringify(dto)}`
    );
    try {
  ...
  async editAlbum(@Body() dto: updateAlbumDto) {
    this.logger.log(
      `Received request to edit album with data: ${JSON.stringify(dto)}`
    );
    try {
      const updatedAlbum = await this.albumsService.editAlbum(dto);
      this.logger.log(
        `Successfully updated album: ${JSON.stringify(updatedAlbum)}`
      );
      return updatedAlbum;
    } catch (error) {
  ...
  async getAlbum(@Param('id') id: string) {
    ...
    try {
      ...
      this.logger.error(
        `Failed to get album with ID ${id}: ${error.message}`,
        error.stack
      );
      throw new HttpException(
  ...
  async getAlbums(@Param('id') id: string) {
    this.logger.log(
      `Received request to get albums for photographer ID: ${id}`
    );
    try {
      const albums = await this.albumsService.getAlbums(id);
      this.logger.log(
        `Successfully retrieved albums: ${JSON.stringify(albums)}`
      );
      return albums;
    } catch (error) {
      this.logger.error(
        `Failed to get albums for photographer ID ${id}: ${error.message}`,
        error.stack
      );
      throw new HttpException(
  ...
  async getImages(@Param('id') id: string) {
    ...
    try {
      ...
      this.logger.log(
        `Successfully retrieved images: ${JSON.stringify(images)}`
      );
      return images;
    } catch (error) {
      this.logger.error(
        `Failed to get images for album ID ${id}: ${error.message}`,
        error.stack
      );
      throw new HttpException(
  ...
  async deleteAlbum(@Param('id') id: string) {
    ...
    try {
      ...
      this.logger.error(
        `Failed to delete album with ID ${id}: ${error.message}`,
        error.stack
      );
      throw new HttpException(
  ...
  async addImages(@Body() dto: AlbumImagesDto) {
    this.logger.log(
      `Received request to add images to album with data: ${JSON.stringify(
        dto
      )}`
    );
    try {
      ...
      this.logger.log(
        `Successfully added images to album: ${JSON.stringify(updatedAlbum)}`
      );
      return updatedAlbum;
    } catch (error) {
      this.logger.error(
        `Failed to add images to album: ${error.message}`,
        error.stack
      );
      throw new HttpException(
  ...
  async deleteImage(@Param('id') id: string) {
    ...
    try {
      ...
      this.logger.error(
        `Failed to delete image with ID ${id}: ${error.message}`,
        error.stack
      );
      throw new HttpException(
  ...
  async updateAlbumCover(@Body() dto: UpdateAlbumCoverDto) {
    this.logger.log(
      `Received request to update cover image for album ID: ${dto.albumId}`
    );

    try {
      const updatedAlbum = await this.albumsService.updateAlbumCover(dto);
      this.logger.log(
        `Successfully updated cover image for album ID: ${dto.albumId}`
      );
      return updatedAlbum;
    } catch (error) {
      this.logger.error(
        `Failed to update cover image for album ID: ${dto.albumId}`,
        error.stack
      );
      throw new HttpException(
  ...
  async getPaymentStatus(
    ...
    this.logger.log(
      `Received request to fetch payment status for album ID: ${albumId} and user ID: ${userId}`
    );

    try {
      const status = await this.albumsService.getPaymentStatus(userId, albumId);
      this.logger.log(
        `Successfully fetched payment status for album ID: ${albumId} and user ID: ${userId}`
      );
      return status;
    } catch (error) {
      this.logger.error(
        `Failed to get payment status for album ID ${albumId} and user ID ${userId}: ${error.message}`,
        error.stack
      );
