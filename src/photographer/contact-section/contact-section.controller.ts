...
private readonly logger = new Logger(ContactSectionController.name);
  ...
  async updateContactDetails(@Body() dto: contactDetailsDto) {
    try {
      ...
      const updatedDetails = await this.contactSectionService.updateContactDetails(dto);
  ...
  async getContactDetails(@Param('id') id: string) {
    try {
      ...
      const contactDetails = await this.contactSectionService.getContactDetails(id);
      if (!contactDetails) {
