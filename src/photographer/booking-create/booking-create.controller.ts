...
export class BookingCreateController {
  ...
  async getBookingsCategory(@Param('id') id: string) {
    ...
    try {
      ...
      this.logger.error(
        `Failed to fetch bookings categories for photographer: ${id}`,
        error.stack
      );
  ...
  async getBookingsPackage(@Param('id') id: string) {
    ...
    try {
      ...
      this.logger.error(
        `Failed to fetch bookings package for photographer with ID: ${id}`,
        error.stack
      );
      throw new HttpException(
        'Failed to fetch bookings package',
        HttpStatus.NOT_FOUND
      );
    }
  }
  ...
  async clientBooking(@Body() dto: ClientBookingDto) {
    ...
    try {
      ...
      this.logger.error(
        `Failed to create booking for client with ID: ${dto.clientId}`,
        error.stack
      );
