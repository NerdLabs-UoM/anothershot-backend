...
export class BookingCreateService {
  ...
  async getBookingsPackage(id: string) {
    try {
      this.logger.log(
        `Fetching booking packages for photographer with ID: ${id}`
      );
      ...
      if (packages.length === 0) {
        this.logger.warn(
          `No booking packages found for photographer with ID: ${id}`
        );
      }

      this.logger.log(
        `Successfully fetched booking packages for photographer with ID: ${id}`
      );
      return packages;
    } catch (error) {
      this.logger.error(
        `Failed to fetch booking packages for photographer with ID: ${id}`,
        error.stack
      );
      throw new HttpException(
        'Failed to fetch booking packages',
        HttpStatus.NOT_FOUND
      );
    }
  }
  ...
  async clientBooking(dto: ClientBookingDto) {
    try {
      this.logger.log(
        `Creating booking for client ID: ${dto.clientId} and photographer ID: ${dto.photographerId}`
      );
      ...
      this.logger.log(
        `Successfully created booking for client ID: ${dto.clientId} and photographer ID: ${dto.photographerId}`
      );
      return newBooking;
    } catch (error) {
      this.logger.error(
        `Failed to create booking for client ID: ${dto.clientId} and photographer ID: ${dto.photographerId}`,
        error.stack
      );
      throw new HttpException(
