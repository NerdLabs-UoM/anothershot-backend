...
export class FeedController {
  ...
  async getFeed(@Param('id') id: string) {
    ...
    try {
      ...
      throw new HttpException(
        `Feed not found for photographer with ID: ${id}`,
  ...
  async createFeedComponent(@Body() dto: FeedDto) {
    this.logger.log(
      `Creating feed component for photographer with ID: ${dto.photographerId}`
    );
    try {
      return await this.feedService.createFeedComponent(dto);
    } catch (error) {
      throw new HttpException(
        `Error creating feed component for photographer with ID: ${dto.photographerId}`,
  ...
  async feedLike(
    ...
    try {
      ...
      throw new HttpException(
        `Error liking feed for photographer with ID: ${photographerId}`,
        HttpStatus.BAD_REQUEST
      );
    }
  }
  ...
  async feedSave(
    ...
    try {
      ...
      throw new HttpException(
        `Error saving feed for photographer with ID: ${photographerId}`,
        HttpStatus.BAD_REQUEST
      );
    }
  }
  ...
  async deleteFeed(@Body() dto: DeleteFeedDto) {
    ...
    try {
      ...
      throw new HttpException(
        `Error deleting feed for image with ID: ${dto.feedId}`,
        HttpStatus.BAD_REQUEST
      );
    }
  }
  ...
  async getFeedHeader(@Param('id') id: string) {
    ...
    try {
      ...
      throw new HttpException(
        `Feed header not found for photographer with ID: ${id}`,
        HttpStatus.NOT_FOUND
      );
    }
  }
  ...
  async updateCaption(@Body() dto: CaptionDto) {
    ...
    try {
      ...
      throw new HttpException(
        `Error updating caption for feed with ID: ${dto.feedId}`,
        HttpStatus.BAD_REQUEST
      );
    }
  }
