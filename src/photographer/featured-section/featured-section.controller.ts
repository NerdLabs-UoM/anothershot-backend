...
export class FeaturedSectionController {
  ...
  constructor(
    private readonly featuredSectionService: FeaturedSectionService
  ) {}
  ...
  async getFeatured(@Param('photographerId') id: string) {
    try {
      this.logger.log(
        `Fetching featured photographer for photographer ID: ${id}`
      );
      const featuredPhotographer =
        await this.featuredSectionService.getFeatured(id);
      if (!featuredPhotographer) {
        this.logger.warn(
          `No featured photographer found for photographer ID: ${id}`
        );
      ...
      this.logger.log(
        `Featured photographer fetched successfully for photographer ID: ${id}`
      );
      return featuredPhotographer;
    } catch (error) {
      this.logger.error(
        `Error fetching featured photographer for photographer ID: ${id}`,
        error.message
      );
      throw new HttpException(
        {
  ...
  async updateFeatured(
    @Param('id') id: string,
    @Body() featured: Partial<Photographer>
  ) {
    try {
      this.logger.log(`Updating featured photographer for ID: ${id}`);
      await this.featuredSectionService.updateFeatured(id, featured);
      this.logger.log(
        `Featured photographer updated successfully for ID: ${id}`
      );
    } catch (error) {
      this.logger.error(
        `Error updating featured photographer for ID: ${id}`,
        error.message
      );
      throw new HttpException(
        {
