import { Body, Controller, Delete, Get, Patch, Param, Logger, HttpException, HttpStatus } from '@nestjs/common';
...
export class ClientController {
    ...
    async updateImage(@Body() dto: ClientImageDto) {
        ...
        try {
            ...
            this.logger.log(`Image updated successfully for client with ID: ${dto.clientId}`);
            return result;
        } catch (error) {
            this.logger.error(`Failed to update image for client with ID: ${dto.clientId}`, error.stack);
    ...
    async updateProfile(@Body() dto: ClientDto) {
        ...
        try {
            ...
            this.logger.log(`Profile updated successfully for client with ID: ${dto.clientId}`);
            return result;
        } catch (error) {
            this.logger.error(`Failed to update profile for client with ID: ${dto.clientId}`, error.stack);
            throw new HttpException(error.message, HttpStatus.BAD_REQUEST);
        }
    }
    ...
    async getClientDetails(@Param('id') id: string) {
        ...
        try {
            ...
            this.logger.error(`Failed to fetch details for client with ID: ${id}`, error.stack);
    ...
    async getLikedImages(@Param('id') id: string) {
        ...
        try {
            ...
            this.logger.log(`Fetched liked images successfully for client with ID: ${id}`);
            return result;
        } catch (error) {
            this.logger.error(`Failed to fetch liked images for client with ID: ${id}`, error.stack);
            throw new HttpException(error.message, HttpStatus.NOT_FOUND);
        }
    }
    ...
    async getSavedImages(@Param('id') id: string) {
        ...
        try {
            ...
            this.logger.log(`Fetched saved images successfully for client with ID: ${id}`);
            return result;
        } catch (error) {
            this.logger.error(`Failed to fetch saved images for client with ID: ${id}`, error.stack);
            throw new HttpException(error.message, HttpStatus.NOT_FOUND);
        }
    }
    ...
    async getBookings(@Param('id') clientId: string) {
        ...
        try {
            ...
            this.logger.log(`Fetched bookings successfully for client with ID: ${clientId}`);
            return result;
        } catch (error) {
            this.logger.error(`Failed to fetch bookings for client with ID: ${clientId}`, error.stack);
            throw new HttpException(error.message, HttpStatus.NOT_FOUND);
        }
    }
    ...
    async deleteBooking(@Body() dto: DeleteBookingDto) {
        this.logger.log(`Deleting booking for client with booking ID: ${dto.bookingId}`);
        try {
            const result = await this.clientService.deleteBooking(dto);
            this.logger.log(`Booking deleted successfully for client with booking ID: ${dto.bookingId}`);
            return result;
        } catch (error) {
            this.logger.error(`Failed to delete booking for client with booking ID: ${dto.bookingId}`, error.stack);
            throw new HttpException(error.message, HttpStatus.BAD_REQUEST);
        }
    }
}
