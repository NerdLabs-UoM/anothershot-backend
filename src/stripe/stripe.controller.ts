...
export class StripeController {
  ...
  async handleStripeEvent(
    ...
    try {
      ...
      this.logger.error('⚠️  Webhook signature verification failed.', error.message);
    ...
    switch (event.type) {
      ...
      case 'checkout.session.completed':
        ...
        this.logger.log(`Checkout session completed with metadata: ${JSON.stringify(checkoutSession.metadata)}`);
        ...
        if (checkoutSession.metadata.bookingId != null) {
          try {
            ...
            throw new InternalServerErrorException('Failed to process booking payment');
          }
          ...
          try {
            ...
            throw new InternalServerErrorException('Failed to process album payment');
          }
        }
        break;
